...
EvalNet::EvalNet(int ninputs, int nhidden, bool dlinks, unsigned theseed, float lowrange, float highrange, float bias, char* n)
{
   SetBias(bias);
...
EvalNet::EvalNet(char* loadfile, float bias)
{
...
void EvalNet::LoadWeights(char* loadfile)
{
	float dummy;
   char buf[64];
   ifstream nf;
...
void EvalNet::GetName(char* n)
{
	strcpy(n, name);
}

void EvalNet::SetName(char* n)
{
	strcpy(name, n);
}
...
void EvalNet::InitTDTrain(float* iv, float gamma, float lambda)
{
	InitElig();
	InitMomentum();
	InitError();
...
void EvalNet::TDFinal(float evalue)
{
...
float EvalNet::EvaluateNet(float* inputvector)
{
	int i,j;

	//set the BIAS
	inputvector[numinputs] = BIAS;
	...
	for (i=0; i<numhidden; i++)
	{
		...
		{
      		hiddenunits[i] += weights1[j][i] * inputvector[j];
		}
	...
	{
		for (i=0; i<=numinputs; i++)
	   		outputunit += weights3[i] * inputvector[i];
	}
...
void EvalNet::UpdateElig(float* iv)
{
	int 	i,j;
	float deriv;
	...
	{
		for (i=0; i<=numinputs; i++)
		   elig3[i] = (LAMBDA*elig3[i]) + (deriv*iv[i]);
	}
	...
	for (i=0; i<numhidden; i++)
		for (j=0;j<=numinputs;j++)
			elig1[j][i]= (LAMBDA * elig1[j][i]) + (deriv * weights1[j][i] * DERIV(hiddenunits[i]) * iv[j]);
}



/* save the net to a file */
void EvalNet::SaveNet(char* savefile)
{
	int i,j;
 	ofstream sf;
...
void EvalNet::InittoRandom(float lowrange, float highrange)
{
   int 	i,j;			//loopers!
...
void EvalNet::InitError()
{
	maxerr = avgerr = 0.0;
}

/* Initialises net's momentum arrays */
void EvalNet::InitMomentum()
{
	int i,j;
...
float* EvalNet::floatalloc(size_t size)
{
	float* ret;
...
float** EvalNet::pfloatalloc(size_t size)
{
	float** ret;
...
float EvalNet::getrealrandom(int irange)
{
	int r;
   float rfloat;
...
float EvalNet::getmaxerr(void)
{
	return maxerr;
}
float EvalNet::getavgerr(void)
{
	return avgerr;
}
...
void EvalNet::SetLRate1(float lrate)
{
   LRATE1 = lrate;
}

void EvalNet::SetLRate2(float lrate)
{
   LRATE2 = lrate;
}

void EvalNet::SetMomentum(float momentum)
{
	MOMENTUM = momentum;
}

void EvalNet::SetBias(float bias)
{
	BIAS = bias;
}
void EvalNet::SetDecay(float decay)
{
	GAMMA = decay;
}

void EvalNet::SetLambda(float l)
{
	LAMBDA = l;
}

int EvalNet::NumInputs()
{
	return numinputs;
}
int EvalNet::NumHidden()
{
	return numhidden;
}
